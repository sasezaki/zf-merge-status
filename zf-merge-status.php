...
function check($differ, $onlynotice, $echoCommitMessage){
    ...
    foreach ($checks as $rev => $status) {
        ...
        if ($status->status == ZFMerge_Status::STATUS_NONE) {
            ...
            echo "[r$rev](http://framework.zend.com/code/revision.php?repname=zf.Zend+Framework&path=%2Ftrunk&rev=$rev)";  
            ...
            if ($onlynotice) {
                if ($echoCommitMessage) {
                    ...
                    foreach ($status->commit['PATHS'] as $path) {
                        $href = 'http://framework.zend.com/code/diff.php?repname=zf.Zend+Framework&path='. 
